...
$(document).ready(function () {
  function renderPagination(current, total) {
    ...
    const createButton = (pageNum, isActive = false) => {
      ...
      btn.on('click', () => loadAppointments(currentStatus, pageNum));
      return btn;
    };
  ...
  let currentPage = 1;
  const itemsPerPage = 5; // You can adjust per page limit

  function loadAppointments(status, page = 1) {
    currentStatus = status;
    currentPage = page;

    $.ajax({
      url: `/appointments/status/${status}?page=${page}&limit=${itemsPerPage}`,
      method: 'GET',
      success: function (res) {
        ...
        if (totalItems === 0) {
          ...
          html += `<div class="px-3 py-2 fw-bold fs-4">Appointments: ${totalItems}</div>`;
          data.forEach((item, index) => {
            const shortMessage = item.message.length > 200 ? item.message.substring(0, 200) + '...' : item.message;
            let acceptBtn = '';
            if (item.status === 'pending') {
              acceptBtn = item.acceptedOrNot === 'accepted'
                ? `<button class="btn btn-gray btn-sm" disabled>Accepted</button>`
                : `<button class="btn btn-secondary btn-sm accept-btn" data-id="${item._id}">Accept</button>`;
            }

            html += `
              <div class="d-flex flex-row comment-row border-bottom p-3 gap-3 appointment-card" 
                  data-index="${(page - 1) * itemsPerPage + index}">
                <div>
                ...
                <div class="comment-text w-100">
                  ...
                  <div class="comment-footer mt-2 d-flex justify-content-between align-items-center">
                    ...
                    ${status === 'pending'
                      ? `<button class="btn btn-warning btn-sm ms-2 reschedule-btn" data-id="${item._id}">Reschedule</button>`
                      : ''}
                  </div>
  ...
  function updateAppointmentDate(appointmentId, newDate) {
    ...
    Swal.fire({
      title: 'Updating Date...',
      text: 'Notifying doctor, patient & admin...',
      allowOutsideClick: false,
      didOpen: () => {
        Swal.showLoading(); // shows spinner
      }
    ...
    $.ajax({
      url: `/appointments/update-date/${appointmentId}`,
      method: 'POST',
      data: { newDate },
      success: function (res) {
        Swal.fire({
          title: 'Date Updated!',
          text: 'Notifications sent to doctor, patient, and admin.',
          icon: 'success',
          timer: 2500,
          showConfirmButton: false
        });
        loadAppointments(currentStatus); // Reload list
      },
      error: function (err) {
        Swal.fire({
          title: 'Error!',
          text: err.responseJSON?.message || 'Failed to update date.',
          icon: 'error'
        });
      }
    });
  ...
  $('#appointmentList').on('click', '.reschedule-btn', function (e) {
    e.stopPropagation(); // Prevent card popup
    const appointmentId = $(this).data('id');

    Swal.fire({
      title: 'Select New Appointment Date',
      html: `<input type="datetime-local" id="newDate" class="form-control text-black">`,
      confirmButtonText: 'Proceed',
      focusConfirm: false,
      preConfirm: () => {
        const selectedDate = $('#newDate').val();
        if (!selectedDate) {
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          title: 'Are you sure?',
          html: `Changing this appointment date will notify:<br>
                The Doctor,<br>
                The Patient. and<br>
                You (The Admin)<br><br>
                <b>New Date:</b> ${formatDate(selectedDate)}`,
          icon: 'warning',
          showCancelButton: true,
          confirmButtonText: 'Yes, Change & Notify',
          cancelButtonText: 'Cancel'
        }).then((confirmResult) => {
          if (confirmResult.isConfirmed) {
            updateAppointmentDate(appointmentId, selectedDate);
          }
        });
      }
    });
  ...
  // Accept appointment and forward necessary messages
  $('#appointmentList').on('click', '.accept-btn', function (e) {
    e.stopPropagation();
    const appointmentId = $(this).data('id');
    ...
    Swal.fire({
      ...
      html: `
        <p>Do you want to <strong>accept</strong> this appointment?</p>
        <p>This will notify:</p>
          The Doctor,<br>
          The Patient. and<br>
          You (The Admin)<br><br>
        <p>Appointment will be set with the current date. Unless you wish to reschedule.</p>
      `,
      icon: 'question',
      showCancelButton: true,
      confirmButtonText: 'Yes, Accept & Notify',
      cancelButtonText: 'Cancel'
    }).then((result) => {
      if (result.isConfirmed) {

        // ✅ Show loading spinner while AJAX is running
        Swal.fire({
          title: 'Processing...',
          text: 'Notifying doctor, patient & admin...',
          allowOutsideClick: false,
        ...
        $.ajax({
          ...
          error: function (err) {
            Swal.fire({
              title: '❌ Error!',
              text: err.responseJSON?.message || 'Failed to accept appointment.',
              icon: 'error'
            });
          }
        });
...
$(document).ready(function() {
  ...
  // Preview for Create
  $('#deptImage').on('change', function (event) {
    const preview = $('#previewDeptPic');
  ...
  $('#departmentForm').on('submit', function(e) {
    ...
    $.ajax({
      ...
      $('#previewDeptPic').addClass('d-none').attr('src', '');
      fetchDepartments();
    })
    .fail(function(xhr) {
      Swal.fire({
  ...
  function fetchDepartments() {
    $.get('/departments/getAllDepartments', function(data) {
      const deptContainer = $('.dept-data');
      deptContainer.empty();

      if (data.length === 0) {
        deptContainer.append(`
          <div class="text-center text-muted">No departments found.</div>
        `);
        return;
      }
      ...
      data.forEach((dept, index) => {
        ...
        const imageUrl = dept.imageUrl || '/dashboard-assets/images/products/s1.jpg';

        deptContainer.append(`
          <div class="d-flex align-items-center justify-content-between p-3 mb-3 border rounded shadow-sm">
            <div class="d-flex align-items-center gap-3">
              <img src="${imageUrl}" alt="Dept Img"
                class="rounded-circle border"
                style="width: 50px; height: 50px; object-fit: cover;">
              <div>
                <div class="fw-bold text-black">${dept.name}</div>
                <div class="text-muted small">${shortMessage || 'None'}</div>
              </div>
            </div>
            <div>
              <a href="javascript:void(0)" class="text-primary me-2 edit-dept" data-id="${dept._id}">
                <i class="ti ti-edit fs-5"></i>
              </a>
              <a href="javascript:void(0)" class="text-danger delete-dept" data-id="${dept._id}">
  ...
  $(document).on('click', '.edit-dept', function () {
    ...
    $.get('/departments/getSingleDepartment/' + id, function (dept) {
      ...
      if (dept.imageUrl) {
        $('#currentDeptPic').attr('src', dept.imageUrl);
      }
  ...
  // Preview for Edit
  $('#editDeptPic').on('change', function (event) {
    const preview = $('#previewEditDeptPic');
    const file = event.target.files[0];
    if (file) {
      preview.attr('src', URL.createObjectURL(file)).removeClass('d-none');
    } else {
  ...
  $('#editDeptForm').on('submit', function(e) {
    ...
    .done(() => {
      ...
      $('#previewEditDeptPic').addClass('d-none').attr('src', '');
      fetchDepartments();
    })
    .fail((xhr) => {
...
$(document).ready(function() {
        ...
        cat.doctors.forEach(doc => {
          const html = `
            <div class="d-flex flex-row comment-row border-bottom p-3 gap-3 align-items-center">
              <div>
                <span><img src="${doc.profilePic || '/dashboard-assets/images/profile/user-3.jpg'}" class="rounded-circle doctor-avatar" width="50" height="50" alt="doctor" /></span>
              </div>
              <div class="comment-text w-100">
                ...
                <div class="comment-footer mt-2 d-flex align-items-center justify-content-between">
                  ...
                  <span class="action-icons">
                    <a href="javascript:void(0)" class="ps-2 edit-doctor" data-id="${doc._id}" data-department="${cat._id}" data-name="${doc.name}" data-email="${doc.email || ''}" data-phone="${doc.phone || ''}" data-pic="${doc.profilePic || ''}">
                      <i class="ti ti-edit fs-5"></i>
                    </a>
  ...
  $('#doctorForm').on('submit', function (e) {
    ...
    // Basic validation
    if (!$('#name').val().trim() || !$('#department').val().trim() || !$('#email').val().trim() || !$('#phone').val().trim()) {
      return Swal.fire({
        icon: 'warning',
    ...
    $.ajax({
      ...
      success: function (response) {
        Swal.fire({
          ...
          $('#doctorForm')[0].reset(); // ✅ Reset the form

          $('#previewDoctorPic').addClass('d-none').attr('src', '');// ✅ Hide the preview image
        });
      },
      error: function (xhr) {
        Swal.fire({
  ...
  $(document).on('click', '.edit-doctor', function () {
    ...
    const profilePic = $(this).data('pic'); // <-- add this to your doctor list buttons
    ...
    // Show existing picture (fallback to a placeholder)
    $('#currentDoctorPic').attr('src', profilePic || '/default-avatar.jpg');

    // Reset preview for new selection
    $('#previewDoctorPic').addClass('d-none').attr('src', '');
  ...
  $('#profilePic, #editDoctorPic').on('change', function (e) {
    ...
    reader.onload = function (event) {
      if (e.target.id === 'profilePic') {
        // Add Doctor form preview
        $('#previewDoctorPic').attr('src', event.target.result).removeClass('d-none');
      } else if (e.target.id === 'editDoctorPic') {
        // Edit Doctor modal preview
        $('#previewEditDoctorPic').attr('src', event.target.result).removeClass('d-none');
      }
    };
  ...
  $('#editDoctorForm').on('submit', function (e) {
    ...
    $.ajax({
      ...
      success: function () {
        ...
        $('#editDoctorPic').val(''); // Clear the file input
        $('#previewEditDoctorPic').attr('src', '').addClass('d-none'); // Hide preview
      },
      error: function (xhr) {
...
$(document).ready(function () {
  ...
  $(document).on('click', '.edit-carousel', function () {
    ...
    $.get(`/images/carousel/getCarouselImageById/${id}`, function (data) {
      ...
      $('#editCarouselDescription').val(data.description);
            ...
            return `
              <div class="d-flex flex-row comment-row border-bottom p-3 gap-3">
                ...
                <div class="comment-text w-100">
                  ...
                  <div class="comment-footer mt-2 d-flex justify-content-between align-items-center">
                    <div>
                      <a href="javascript:void(0)" class="ps-3 text-primary edit-blog-btn" data-id="${blog._id}">
                        <i class="ti ti-edit fs-5"></i>
                      </a>
...
$(document).ready(function () {
  ...
  if (window.location.pathname === '/dashboard/services') {
    // Preview selected new image
    ...
    $('#serviceForm').submit(function (e) {
        ...
        // Safely get formatted content if editor is ready
        if (createEditor) {
          $('#serviceDescription').val(createEditor.getData());
        }
        const formEl = this;
        const fd = new FormData(formEl);
        ...
        $.ajax({
            ...
            error(err) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: err.responseJSON?.message || 'Error creating service'
                });
            }
        });
    });
    ...
    function fetchServices(page = 1) {
      $.get(`/services/getAllServices?page=${page}&limit=${itemsPerPage}`, function (res) {
        ...
        if (services.length === 0) {
          ...
          html = services.map(service => {
            const shortMessage = service.description.length > 50 ? service.description.substring(0, 50) + '...' : service.description;

            return `
              <div class="d-flex flex-row comment-row border-bottom p-3 gap-3">
                <div>
                  ${service.image
                    ? `<img src="${service.image}" class="rounded-circle" width="50" height="50" />`
                ...
                <div class="comment-text w-100">
                  ...
                  <div class="comment-footer mt-2 d-flex justify-content-between align-items-center">
                    <div>
                      <a href="javascript:void(0)" class="ps-3 text-primary edit-btn" data-id="${service._id}">
                        <i class="ti ti-edit fs-5"></i>
                      </a>
                      <a href="javascript:void(0)" class="ps-3 text-danger delete-btn" data-id="${service._id}">
                        <i class="ti ti-trash fs-5"></i>
                      </a>
                    </div>
                    <span class="text-muted fw-normal fs-2">${new Date(service.createdAt).toDateString()}</span>
                  </div>
                </div>
              </div>
            `;
        ...
        renderPagination(currentPage, totalPages, fetchServices);
      });
    }
    ...
    $(document).on('click', '.edit-btn', function () {
      const id = $(this).data('id');
      $.get(`/services/getSingleService/${id}`, function (service) {
        $('#editId').val(service._id);
        $('#editTitle').val(service.title);
        // Load description into CKEditor
        if (editEditor) {
        ...
        if (service.image) {
          $('#currentImage').attr('src', service.image).show();
        } else {
          $('#currentImage').hide();
        }
        $('#previewEditImage').hide().attr('src', '');
    ...
    $('#editForm').submit(function (e) {
      e.preventDefault();
      if (editEditor) {
        $('#editDescription').val(editEditor.getData());
      }
      const id = $('#editId').val();
      const fd = new FormData(this);
      ...
      $.ajax({
        url: `/services/updateSingleService/${id}`,
        type: 'POST',
        data: fd,
        processData: false,
        contentType: false,
        ...
        error(err) {
          ...
          Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: err.responseJSON?.message || 'Error updating service'
          });
        }
      });
    });
    ...
    $(document).on('click', '.delete-btn', function () {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          $.post(`/services/deleteSingleService/${id}`, function () {
            fetchServices();
            Swal.fire(
              'Deleted!',
